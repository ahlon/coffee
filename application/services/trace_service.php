...
require_once APPPATH. 'libraries/audit.php';
/**
 * @author ahlon
 */
class Trace_service extends Base_service {
    ...
    function save($trace) {
        ...
        $table_name = 'trace_'.date('Ymd', strtotime($trace['time']));
        $this->trace_model->set_table_name($table_name);
        $this->trace_model->save($trace_log);
    }
    /**
     * 获取trace数据
    ...
    function get_trace_data($conditions, $time, $page_size, $page){
        $reuslt = $this->trace_model->get_trace_data($conditions, $time, $page_size, $page);
...
//         }
    ...
    function get_trace_data_count($conditions,$time){
        return $this->trace_model->get_trace_data_count($conditions,$time);
    }
    /**
     * 获取ONE数据记录条数
     * @param unknown_type $conditions
    ...
    function get_one_trace_data($conditions,$time){
        return $this->trace_model->get_one_trace_data($conditions,$time);
    }
    
    function get_latest_view_products($user_id, $count = 10) {
        $params = array('uid'=>$user_id, 'controller'=>'product3', 'method'=>'view');
        return $this->trace_model->find_all($params, 'id desc', $count, 1);
    }
}